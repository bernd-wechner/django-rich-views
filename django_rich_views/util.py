...
# Python imports
import re
# Specifically to avoid a PyDev Error in the IDE.
...
def is_to_many(field):
    ''' A quick way to determine if a field, returned by getattr(model, field_name) is a to_many field. '''
    if hasattr(field, "rel"):
        return field.rel.many_to_many or field.rel.one_to_many
    else:
        return False
...
def getApproximateArialStringWidth(st):
    ...
    for s in st:
        if s in 'lij|\' ':
            ...
            size += 85
        elif s in 'aebdhnopqug#$L+<>=?_~FZT1234567890':
            size += 95
...
def emulatePRE(string, indent=odm.indent):
    ...
    tag_start = r"<SPAN style='display: block; font-family: monospace; white-space: pre; padding-left:{}ch; margin-top=0;'>".format(
        indent)
    tag_end = r"</SPAN>"
...
def indentVAL(string, indent=odm.indent):
    ...
    if indent > 0:
        tag_start = r"<SPAN style='display: block; padding-left:{}ch; margin-top=0; margin-bottom=0; padding-bottom=0;'>".format(
            indent)
        tag_end = r"</SPAN>"
...
def pythonify(json_data):
    ...
    for key, value in json_data.items():
        if isinstance(value, list):
            value = [pythonify(item) if isinstance(
